...
class IRLRewardModelWorker(Worker):
    ...
    def init_model(self):
        ...
        from .irl_dp_rm import DataParallelIRLRewardModel
        self.reward_module, self.reward_optimizer, self.reward_lr_scheduler = self._build_reward_model_and_optimizer(config=self.config)
        ...
        self.rm = DataParallelIRLRewardModel(config=self.config, reward_module=self.reward_module, reward_optimizer=self.reward_optimizer)
    ...
    def update_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics = self.rm.update_rm(data=data)

            self.reward_lr_scheduler.step()
            lr = self.reward_lr_scheduler.get_last_lr()[0]
            metrics['rm/lr'] = lr

            output = DataProto.from_dict(tensors={"placeholder": torch.tensor([0.0])}, meta_info={'metrics': metrics})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=3):
        import torch
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.reward_module)

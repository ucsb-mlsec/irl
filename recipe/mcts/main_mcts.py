...
"""
from .mcts_ray_trainer import RayMCTSTrainer
...
@hydra.main(config_path='config', config_name='mcts_trainer', version_base=None)
def main(config):
    run_mcts(config)


def run_mcts(config, compute_score=None):
...
def main_task(config, compute_score=None):
    ...
    if config.reward_model.enable:
        from .mcts_fsdp_workers import MCTSRewardModelWorker
        role_worker_mapping[Role.RewardModel] = ray.remote(MCTSRewardModelWorker)
    ...
    trainer = RayMCTSTrainer(config=config,
                            tokenizer=tokenizer,

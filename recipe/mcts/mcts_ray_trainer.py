...
def compute_advantage(data: DataProto, adv_estimator, config):
    ...
    advantages, returns = mcts_core_algos.compute_advantage_return(data, response_mask, config.actor_rollout_ref.rollout.n, config)
...
class RayMCTSTrainer(RayPPOTrainer):
    """
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for policy_batch_dict, expert_batch_dict in zip(self.policy_train_dataloader, self.expert_demo_dataloader):
                ...
                keep_n_policy = int(len(policy_batch) // 2)
                policy_batch.reorder(filter_reorder_index[:keep_n_policy])
                ...
                # Step 3: Compute reward scores only for policy batch
                rm_scores = self.rm_wg.compute_rm_score(policy_batch).batch['rm_scores']
                policy_rollout_rm_scores = DataProto.from_dict({'rm_scores': rm_scores})
                ...
                # Collect metrics
                metrics.update(compute_timing_metrics(batch=policy_batch, timing_raw=timing_raw))

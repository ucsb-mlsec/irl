...
class PRIMERewardModelWorker(Worker):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            expert = torch.tensor(acc.clone().detach()==1, dtype=torch.bool)
            policy = torch.tensor(acc.clone().detach()==0, dtype=torch.bool)

            expert_rm_scores = rm_scores[expert]
            policy_rm_scores = rm_scores[policy]

            expert_response_mask = response_mask[expert]
            policy_response_mask = response_mask[policy]

            expert_rm_scores = (expert_rm_scores * expert_response_mask).sum().item()
            policy_rm_scores = (policy_rm_scores * policy_response_mask).sum().item()

            metrics['reward_model/avg_expert_rm_scores'] = expert_rm_scores
            metrics['reward_model/avg_policy_rm_scores'] = policy_rm_scores
